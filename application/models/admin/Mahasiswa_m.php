...
class Mahasiswa_m extends CI_Model{
    ...
    public function det_mhs($id){
        $this->db->select('mahasiswa_pt.*,mahasiswa.*,users.username,users.password,users.repassword,sms.id,sms.id_sms,sms.nm_lemb,sms.kode_prodi,mahasiswa_pt.id AS idmhspt,mahasiswa.id AS idmhs,mahasiswa.email AS email_mhs');
        $this->db->where('mahasiswa_pt.id',$id);
        $this->db->join('mahasiswa', 'mahasiswa.id = mahasiswa_pt.id_pd_siakad');
        $this->db->join('sms', 'sms.kode_prodi = mahasiswa_pt.kode_sms');
        $this->db->join('users', 'users.id_mhs = mahasiswa.id');
